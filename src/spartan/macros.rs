...
macro_rules! zip_with {
    ...
    //   iter1.zip(iter2.zip(iter3)).map(|(a, (b, c))| a + b + c)
    // ```
    //
    // iterator projection specified: use it on each argument
    // ```ignore
    // zip_with!(par_iter, (vec1, vec2, vec3), |a, b, c| a + b + c) ->
    //   vec1.par_iter().zip(vec2.par_iter().zip(vec3.par_iter())).map(|(a, (b, c))| a + b + c)
    // ````
    ...
    //   iter1.zip(iter2.zip(iter3)).for_each(|(a, (b, c))| a + b + c)
    // ```
    //
    //
    // iterator projection specified: use it on each argument
    ...
    //   vec1.part_iter().zip(vec2.par_iter().zip(vec3.par_iter())).for_each(|(a, (b, c))| a + b + c)
    // ```
...
macro_rules! zip_with_for_each {
    ...
    //   vec1.par_iter().zip(vec2.par_iter().zip(vec3.par_iter())).for_each(|(a, (b, c))| a + b + c)
    // ````
    ($($f:ident,)? ($e:expr $(, $rest:expr)*), $($move:ident)? |$($i:ident),+ $(,)?| $($work:tt)*) => {{
