...
use crate::{
  ...
  traits::{
    commitment::{CommitmentEngineTrait, CommitmentTrait, ZKCommitmentEngineTrait, Len},
  ...
  E::GE: DlogGroup<ScalarExt = E::Scalar>,
{
  ...
  fn setup(label: &'static [u8], n: usize) -> Self::CommitmentKey {
    const SUFFIX: &[u8] = b"blinding factor";

    // Create a static array with the combined label and suffix
    let mut blinding_label = Vec::with_capacity(label.len() + SUFFIX.len());
    blinding_label.extend_from_slice(label);
    blinding_label.extend_from_slice(SUFFIX);
...
impl<E> ZKCommitmentEngineTrait<E> for CommitmentEngine<E>
where
  E: Engine,
  E::GE: DlogGroup<ScalarExt = E::Scalar>,
{ 
  ...
  E::GE: DlogGroup,
{
  ...
  fn fold(&self, L: &Self, R: &Self, w1: &E::Scalar, w2: &E::Scalar) -> Self;
