...
use crate::{
  ...
  provider::{
    pedersen::{
      ...
      CommitmentKey as PedersenCommitmentKey, CompressedCommitment as PedersenCompressedCommitment,
    },
    traits::DlogGroup,
  },
...
use super::pedersen::CommitmentEngineExtTrait;
...
pub(in crate::provider) struct InnerProductInstance<E: Engine> {
  comm_a_vec: PedersenCommitment<E>,
  b_vec: Vec<E::Scalar>,
  c: E::Scalar,
}
...
impl<E> TranscriptReprTrait<E::GE> for InnerProductInstance<E>
where
  E: Engine,
  E::GE: DlogGroup,
{
...
pub(in crate::provider) struct InnerProductWitness<E: Engine> {
  a_vec: Vec<E::Scalar>,
}

impl<E: Engine> InnerProductWitness<E> {
  pub(in crate::provider) fn new(a_vec: &[E::Scalar]) -> Self {
    Self {
      a_vec: a_vec.to_vec(),
    }
  }
...
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(bound = "")]
pub struct InnerProductArgument<E>
where
  E: Engine,
  E::GE: DlogGroup<ScalarExt = E::Scalar>,
{
  L_vec: Vec<PedersenCompressedCommitment<E>>,
  R_vec: Vec<PedersenCompressedCommitment<E>>,
  a_hat: E::Scalar,
}
  ...
  E::GE: DlogGroup<ScalarExt = E::Scalar>,
{
  ...
  pub(in crate::provider) fn prove(
    ck: &PedersenCommitmentKey<E>,
    ck_c: &PedersenCommitmentKey<E>,
    ...
    let (ck, _) = PedersenCommitmentEngine::<E>::split_at(ck, U.b_vec.len());
    ...
    let ck_c = PedersenCommitmentEngine::<E>::scale(ck_c, &r);
    ...
    let prove_inner = |a_vec: &[E::Scalar],
     ...
     -> Result<
      (
        PedersenCompressedCommitment<E>,
        PedersenCompressedCommitment<E>,
        Vec<E::Scalar>,
        Vec<E::Scalar>,
        PedersenCommitmentKey<E>,
      ),
      NovaError,
    > {
      let n = a_vec.len();
    ...
    let mut L_vec: Vec<PedersenCompressedCommitment<E>> = Vec::new();
    let mut R_vec: Vec<PedersenCompressedCommitment<E>> = Vec::new();
  ...
  pub(in crate::provider) fn verify(
    ...
    let P = U.comm_a_vec + PedersenCommitmentEngine::<E>::commit(&ck_c, &[U.c]);
    ...
    let ck_hat = {
      let c = PedersenCommitmentEngine::<E>::commit(&ck, &s);
      PedersenCommitmentEngine::<E>::reinterpret_commitments_as_ck(&[c])
    };
    ...
    let P_hat = {
      let ck_folded = {
        let L_vec_decomp = self
          .L_vec
          .iter()
          .map(|L| PedersenCommitment::<E>::decompress(L))
          .collect::<Result<Vec<_>, _>>()?;
        let R_vec_decomp = self
          .R_vec
          .iter()
          .map(|R| PedersenCommitment::<E>::decompress(R))
          .collect::<Result<Vec<_>, _>>()?;

        let ck_L = PedersenCommitmentEngine::<E>::reinterpret_commitments_as_ck(&L_vec_decomp);
        let ck_R = PedersenCommitmentEngine::<E>::reinterpret_commitments_as_ck(&R_vec_decomp);
        let ck_P = PedersenCommitmentEngine::<E>::reinterpret_commitments_as_ck(&[P]);
        PedersenCommitmentEngine::combine(&PedersenCommitmentEngine::combine(&ck_L, &ck_R), &ck_P)
      };

      PedersenCommitmentEngine::<E>::commit(
        &ck_folded,
        &r_square
          .iter()
    ...
    if P_hat
      == PedersenCommitmentEngine::<E>::commit(
        &PedersenCommitmentEngine::<E>::combine(&ck_hat, &ck_c),
