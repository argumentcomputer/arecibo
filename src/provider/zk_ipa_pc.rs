...
use crate::{
  ...
  errors::NovaError,
  provider::{traits::DlogGroup, zk_pedersen::CommitmentKeyExtTrait },
  spartan::polys::eq::EqPolynomial,
  traits::{
    commitment::{CommitmentEngineTrait, CommitmentTrait, Len},
    zkevaluation::{EvaluationEngineTrait, GetEvalCommitmentsTrait},
  ...
  CommitmentKey<E>: CommitmentKeyExtTrait<E>,
{
  ...
  fn prove_batch(
    ...
    comm: &[Commitment<E>],
    polys: &[Vec<E::Scalar>],
    blind_polys: &[E::Scalar],
    points: &[Vec<E::Scalar>],
    evals: &[E::Scalar],
    blind_evals: &[E::Scalar],
    comm_evals: &[CompressedCommitment<E>],
  ) -> Result<Self::EvaluationArgument, NovaError> {
    // sanity checks (these should never fail)
    assert!(polys.len() >= 2);
    assert_eq!(comm.len(), polys.len());
    assert_eq!(comm.len(), points.len());
    assert_eq!(comm.len(), evals.len());
    assert_eq!(evals.len(), blind_evals.len());
    ...
    let mut U_folded = InnerProductInstance::new(
      ...
      &Commitment::<E>::decompress(&comm_evals[0])?,
    );

    comms_y_vec.push(comm_evals[0].clone());

    // Record value of eval and randomness used in commitment in the witness
    let mut W_folded =
      InnerProductWitness::new(&polys[0], &blind_polys[0], &evals[0], &blind_evals[0]);
    ...
    for i in 1..polys.len() {
      ...
      let (n, u, w) = NIFSForInnerProduct::prove(
        ...
        &InnerProductInstance::new(
          ...
          &Commitment::<E>::decompress(&comm_evals[i])?,
        ),
        &InnerProductWitness::new(&polys[i], &blind_polys[i], &evals[i], &blind_evals[i]),
        transcript,
      );

      nifs.push(n);
      comms_y_vec.push(comm_evals[i].clone());
  ...
  E::GE: DlogGroup<ScalarExt = E::Scalar>,
{
    ...
    CommitmentKey<E>: CommitmentKeyExtTrait<E>,
  {
    ...
    let comm_cross = CE::<E>::zkcommit(&pk.ck_s, &[cross_term], &r_cross);
    ...
    let comm_y = CE::<E>::zkcommit(&pk.ck_s, &[y], &r_y);
  ...
  CommitmentKey<E>: CommitmentKeyExtTrait<E> + Len,
{
  ...
  > {
    let n = x_vec.len();
    let (gens_L, gens_R) = ck.clone().split_at(n / 2);
    ...
    let (L, R) = ck.clone().split_at(ck.length() / 2);

    let gens_folded = ck.fold(&L, &R, &chal_inverse, &chal);
  ...
  pub fn prove(
    ...
    let delta = CE::<E>::zkcommit(&c_hat, &[d], &r_delta).compress();
    let beta = CE::<E>::zkcommit(&ck_y, &[d], &r_beta).compress();
